...
FILE = ["response_time", "waiting_time"]

for z in range(0, 2):
    df = pd.read_csv(os.path.join(os.path.dirname(__file__), "./../supermarket/2kr/P=-1_K=-1_AM=-1_SM=-1/"+FILE[z]+".csv"), sep=';')
    ...
    data.to_csv(os.path.join(os.path.dirname(__file__),
                             "./../supermarket/2kr/P=-1_K=-1_AM=-1_SM=-1/"+FILE[z]+"_out.csv"), sep=';')
